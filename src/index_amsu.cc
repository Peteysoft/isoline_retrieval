...
int main(int argc, char ** argv) {
  ...
  if (argc-optind < 2) {
    printf("usage: index_amsu [-R] [-s] [-f binfile] [-p basepath]\n");
  ...
  for (long i=0; i<nfiles; i++) {
    ...
    (*amsu_head_reader)(filename, tstart[i], tend[i], np, nchan, swap_flag);
  }
  ...
  for (long ifile=0; ifile<nfiles; ifile++) {
    ...
    for (long i=0; i<data->nscan; i++) {
      ...
      for (long j=0; j<data->np; j++) {
        latind=(long) ((data->data[i].lat[j]+90.-offset)/(180.-2*offset)*nlat+1);
        lonind=(long) ((data->data[i].lon[j]+lon_offset)/360.*nlon[latind]);
	if (lonind == nlon[latind]) lonind=nlon[latind]-1;
	binind[j]=lonind*(nlat+2)+latind;
	
        lonind=(long) ((data->data[i].lon[j]+lon_offset)/360.*nlon[latind]-fudge_factor);
	if (lonind < 0) lonind=0;
	if (lonind == nlon[latind]) lonind=nlon[latind]-1;
	binind[j+data->np*7]=lonind*(nlat+2)+latind;

        lonind=(long) ((data->data[i].lon[j]+lon_offset)/360.*nlon[latind]+fudge_factor);
	if (lonind >= nlon[latind]) lonind=nlon[latind]-1;
	binind[j+data->np*8]=lonind*(nlat+2)+latind;

        latind=(long) ((data->data[i].lat[j]+90.-offset)/(180.-2*offset)*nlat-fudge_factor+1);
        lonind=(long) ((data->data[i].lon[j]+lon_offset)/360.*nlon[latind]);
	if (lonind == nlon[latind]) lonind=nlon[latind]-1;
	binind[j+data->np]=lonind*(nlat+2)+latind;
	
        lonind=(long) ((data->data[i].lon[j]+lon_offset)/360.*nlon[latind]-fudge_factor);
	if (lonind < 0) lonind=0;
	if (lonind == nlon[latind]) lonind=nlon[latind]-1;
	binind[j+data->np*2]=lonind*(nlat+2)+latind;

        lonind=(long) ((data->data[i].lon[j]+lon_offset)/360.*nlon[latind]+fudge_factor);
	if (lonind >= nlon[latind]) lonind=nlon[latind]-1;
	binind[j+data->np*3]=lonind*(nlat+2)+latind;

        latind=(long) ((data->data[i].lat[j]+90.-offset)/(180.-2*offset)*nlat+fudge_factor+1);
        lonind=(long) ((data->data[i].lon[j]+lon_offset)/360.*nlon[latind]);
	if (lonind == nlon[latind]) lonind=nlon[latind]-1;
	binind[j+data->np*4]=lonind*(nlat+2)+latind;

        lonind=(long) ((data->data[i].lon[j]+lon_offset)/360.*nlon[latind]-fudge_factor);
	if (lonind < 0) lonind=0;
	if (lonind == nlon[latind]) lonind=nlon[latind]-1;
	binind[j+data->np*5]=lonind*(nlat+2)+latind;

        lonind=(long) ((data->data[i].lon[j]+lon_offset)/360.*nlon[latind]+fudge_factor);
	if (lonind >= nlon[latind]) lonind=nlon[latind]-1;
	binind[j+data->np*6]=lonind*(nlat+2)+latind;
      }

      //sort the bin indices so we can throw away duplicates:
      heapsort_inplace(binind, data->np*9);
