...
nlev=11
r=[indgen(nlev)*155/(nlev-1)+100, 100]
g=r
b=r
...
contour, con, lonret, latret, levels=findgen(nlev)/10-1, /fill, $
	c_color=reverse(indgen(nlev))+1, xrange=[-95, -65], yrange=[45, 70], $
...
device, filename="detail_legend2.ps"

device, /landscape, ysize=6.5, xsize=2, /inches

x=-findgen(nlev)/(nlev-1)
l=rebin(x, nlev, 2)
contour, transpose(l), [0, 1], x, levels=-x-1, xstyle=4, ytitle="R", $
		c_color=reverse(indgen(nlev))+1, /fill
